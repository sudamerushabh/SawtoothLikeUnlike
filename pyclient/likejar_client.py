...
# ------------------------------------------------------------------------------
'''
likeJarClient class interfaces with Sawtooth through the REST API.
...
class LikeJarClient(object):
    '''Client like Jar class

    Supports "like", "unlike", and "count" functions.
    '''
    ...
    def __init__(self, base_url, key_file=None):
        ...
        # Address is 6-char TF prefix + hash of "mylikejar"'s public key
    ...
    def like(self, amount):
        '''like amount likes for the like jar.'''
        return self._wrap_and_send("like", amount, wait=10)

    def unlike(self, amount):
        '''Eat amount likes from the like jar.'''
        try:
            ret_amount = self._wrap_and_send("unlike", amount, wait=10)
        except Exception:
            raise Exception('Encountered an error during unlike')
        return ret_amount

    def count(self):
        '''Count the number of likes in the like jar.'''
    ...
    def _send_to_rest_api(self, suffix, data=None, content_type=None):
        '''Send a REST command to the Validator via the REST API.
           ...
           The latter caller is made on the behalf of like() & unlike().
        '''
